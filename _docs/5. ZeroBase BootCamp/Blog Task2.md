...
|Spring Boot 의 장점||
|-|-|
|의존성 관리 & 자동설정| `@SpringBootApplication`은 크게 세가지 기능<br> `@ComponentScan`,`@EnableAutoConfiguration`,`@SpringBootConfiguration`<br> 을 합쳐놓은 것과 같고 자동으로 빈을 등록, 관리하여 어플리케이션에 반영함|
